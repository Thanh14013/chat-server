...
- `/create <room_name>`
  Tạo một phòng chat mới. Ví dụ: `/create dev_team`
  
- `/join <room_name>`
  Tham gia vào một phòng chat. Ví dụ: `/join dev_team`
  *(Lưu ý: Khi gia nhập phòng mới, bạn sẽ không còn nhận được tin nhắn từ phòng cũ trừ khi có cấu hình hỗ trợ đa phòng)*
  
- `/leave`
  Rời khỏi phòng hiện tại và quay về phòng mặc định.
...
### Xử phạt & Kiểm soát
- `/mute <username>`
  Cấm người dùng này gửi tin nhắn lên server.
- `/unmute <username>`
  Hủy bỏ lệnh cấm chat, cho phép người dùng nhắn tin trở lại.
- `/kick <username>`
  Đuổi người dùng khỏi server (Họ sẽ bị ngắt kết nối nhưng vẫn có thể đăng nhập lại sau).
- `/ban <username>`
  Cấm vĩnh viễn người dùng. Họ sẽ không thể đăng nhập lại.
- `/unban <username>`
  Gỡ bỏ lệnh cấm vĩnh viễn cho tài khoản.
