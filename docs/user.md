...
- `/create <room_name> <password>`
  Tạo một phòng chat mới, yêu cầu phải thiết lập mật khẩu bảo vệ. Ví dụ: `/create dev_team pass123`
  
- `/join <room_name> <password>`
  Tham gia vào một phòng chat, bắt buộc cung cấp mật khẩu. Ví dụ: `/join dev_team pass123`
...
- `/delete <room_name>`
  Xóa phòng chat. Lệnh này chỉ dành cho **Người tạo phòng (Creator)** hoặc **Admin/Owner**. Khi phòng bị xóa, toàn bộ thành viên đang ở trong phòng đó sẽ bị đuổi (kick) về phòng `#general`.
...
- `/listall`
  Hiển thị danh sách toàn bộ người dùng đang trực tuyến **trên toàn hệ thống Server**.
...
### Lệnh Hệ Thống
- `/help`
  Hiển thị danh sách các lệnh bạn có thể sử dụng (Tùy thuộc vào quyền hạn của bạn).
- `/quit`
...
### Xử phạt & Kiểm soát
- `/mute <username>`
  Cấm người dùng này gửi bất kỳ tin nhắn nào lên server (Kể cả chat nhóm phòng hay nhắn riêng `/msg`). Người dùng vẫn có thể đọc tin nhắn.
- `/unmute <username>`
  Hủy bỏ lệnh cấm chat, cho phép người dùng nhắn tin trở lại.
- `/kick <username> <reason>`
  Đuổi người dùng khỏi **phòng hiện tại** và cưỡng chế đưa họ về phòng `#general`. Đồng thời, User này sẽ bị đưa vào danh sách đen của phòng đó và vĩnh viễn không thể `/join` lại.
- `/unkick <username> <room_name>`
  **(Mới)** Gỡ lệnh cấm tham gia phòng cho một User đã bị kick trước đó.
- `/ban <username> <reason>`
  Cấm vĩnh viễn người dùng ở cấp độ **Toàn bộ Hệ thống**. Hệ thống sẽ khóa và lưu lại cả **Địa chỉ IP** lẫn **Nickname**, đảm bảo không thể dùng tài khoản đó hay IP đó kết nối lại. User sẽ lập tức bị ngắt kết nối.
- `/unban <ip_or_nickname>`
  Gỡ bỏ lệnh cấm vĩnh viễn cho tài khoản hoặc địa chỉ IP.

### Phân quyền & Quản lý
- `/promote <username>`
  Thăng cấp quyền quản trị (Admin) cho một người dùng. **(Lệnh này chỉ dành riêng cho OWNER)**.
- `/demote <username>`
  Hạ cấp một Admin trở về trạng thái User bình thường. **(Lệnh này chỉ dành riêng cho OWNER)**.

### Thông báo Khẩn cấp
- `/broadcast <message>`
  Gửi thông báo khẩn cấp màu đỏ / hệ thống đến **toàn bộ người dùng trên tất cả các phòng**. Thường dùng để thông báo bảo trì.
