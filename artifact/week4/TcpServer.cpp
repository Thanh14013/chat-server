...
bool TcpServer::start(uint16_t port, int maxClients, int threadPoolSize) {
    ...
    AUDIT(AuditEventType::CONNECT, "server", "", "SERVER_START", AuditResult::SUCCESS);
    LOG_INFO("VCS SecureChat Server v1.0 starting...");
...
void TcpServer::acceptLoop() {
    while (m_running) {
        ...
        if (clientFd < 0) {
            if (m_running) LOG_WARN("accept() failed.");
            continue;
        }
        ...
        if (ipStatus == IPStatus::BLOCKED) {
            LOG_WARN("Blocked IP attempted connection: " + ip);
            ::close(clientFd);
            continue;
        }

        {
            ...
            if ((int)m_sessions.size() >= m_maxClients) {
                LOG_WARN("Max clients reached. Rejecting: " + ip);
        ...
        LOG_INFO("New connection from: " + ip + " fd=" + std::to_string(clientFd));
        RateLimiter::instance().addClient(clientFd);
...
void TcpServer::handlePacket(int fd, const Packet& pkt) {
    ...
    switch (type) {
        ...
        case MessageType::MSG_ADMIN_KICK: {
            ...
            if (!j.is_discarded())
                AdminCommands::instance().kick(fd,
                    j.value("target",""), j.value("reason",""));
            break;
        }
        ...
        case MessageType::MSG_ADMIN_BAN: {
            auto j = nlohmann::json::parse(payload, nullptr, false);
            if (!j.is_discarded()) {
                if (j.value("unban", false))
                    AdminCommands::instance().unban(fd, j.value("target",""));
                else
                    AdminCommands::instance().ban(fd, j.value("target",""), j.value("reason",""));
            }
            break;
        }
        case MessageType::MSG_ADMIN_PROMOTE: {
            auto j = nlohmann::json::parse(payload, nullptr, false);
            if (!j.is_discarded()) {
                if (j.value("demote", false))
                    AdminCommands::instance().demote(fd, j.value("target",""));
                else
                    AdminCommands::instance().promote(fd, j.value("target",""));
            }
            break;
        }
...
void TcpServer::handleChatSend(int fd, const Packet& pkt) {
    ...
    if (RateLimiter::instance().checkLimit(fd) == RateCheckResult::RATE_LIMITED) {
    ...
    auto filterOut = MessageFilter::instance().filter(
        parsed.message, fd, sess->nickname());

    if (filterOut.result != FilterResult::PASS) {
        sess->sendPacket(Builder::makeError(ErrorCode::ERR_PERMISSION_DENIED,
    ...
    entry.room      = room;
    entry.message   = filterOut.message;
    entry.msg_type  = HistoryMsgType::CHAT;
    RoomManager::instance().appendHistory(room, entry);

    auto broadcast = Builder::makeChatBroadcast(sess->nickname(), room, filterOut.message);
    broadcastToRoom(room, broadcast, -1);

    AUDIT_D(AuditEventType::MESSAGE, sess->nickname(), room, "CHAT_SEND",
            AuditResult::SUCCESS, sess->ip(), "");
}
...
void TcpServer::onClientDisconnected(int fd) {
    RateLimiter::instance().removeClient(fd);
    ClientSession* sess = getSession(fd);
