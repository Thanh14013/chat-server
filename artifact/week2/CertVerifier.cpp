...
std::string CertVerifier::knownServersPath() const {
    // Use $HOME/.vcs_chat/known_servers.json
    const char* home = std::getenv("HOME");
    std::string dir  = home ? std::string(home) + "/.vcs_chat" : "/tmp/.vcs_chat";

    // Create directory if it doesn't exist (mode 0700 — user-only)
...
std::string CertVerifier::fingerprintOf(const std::string& pubkey_pem) {
    // SHA-256 hash of the PEM bytes → "sha256:<hex>"
...
void CertVerifier::loadFromDisk() {
    if (loaded_) return;
    loaded_ = true; // mark even if file is absent

    std::ifstream f(knownServersPath());
    if (!f.is_open()) return; // first run — empty map
...
void CertVerifier::saveToDisk() const {
    ...
    if (!f.is_open()) return; // best-effort write
    f << j.dump(2) << "\n";
}

// ── verifyServerKey ───────────────────────────────────────────────────────────

CertVerifier::TrustStatus CertVerifier::verifyServerKey(const std::string& host,
                                                          int                port,
    ...
    if (it == known_servers_.end()) {
        // First time seeing this server — trust and save
...
void CertVerifier::saveServerKey(const std::string& host,
                                  int                port,
                                  const std::string& pubkey_pem) {
    std::lock_guard<std::mutex> lock(mutex_);
    loadFromDisk();
...
bool CertVerifier::promptUserOnChange(const std::string& host,
                                       int                port,
                                       const std::string& old_fp,
                                       const std::string& new_fp) {
    std::cout << "\n";
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
    std::cout << "║  ⚠️   WARNING: SERVER KEY FINGERPRINT CHANGED!               ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
