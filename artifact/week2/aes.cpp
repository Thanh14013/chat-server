...
AES256GCM::~AES256GCM() {
    // Scrub key material from memory on destruction
...
std::vector<uint8_t> AES256GCM::encrypt(const std::vector<uint8_t>& plaintext,
    ...
    // Fresh random IV for every message — NEVER reuse
    IVBytes iv = generateIV();

    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (!ctx) throw std::runtime_error("AES256GCM::encrypt: EVP_CIPHER_CTX_new failed");

    // Output buffer: IV + ciphertext (same size as plaintext for GCM) + tag
    std::vector<uint8_t> output;
    output.reserve(IV_LEN + plaintext.size() + TAG_LEN);

    // Prepend IV in plaintext — IV is not secret, only unique
    ...
    try {
        ...
        // Set IV length explicitly (128-bit)
        ...
        // Feed AAD (packet header) — authenticated but not encrypted
        if (!aad.empty()) {
            int aad_len = 0;
...
std::vector<uint8_t> AES256GCM::decrypt(const std::vector<uint8_t>& ciphertext,
    ...
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (!ctx) throw std::runtime_error("AES256GCM::decrypt: EVP_CIPHER_CTX_new failed");
