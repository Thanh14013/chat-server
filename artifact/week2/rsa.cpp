...
void RSA2048::generateKeyPair() {
    ...
    if (!ctx) throw std::runtime_error("RSA2048::generateKeyPair: EVP_PKEY_CTX_new_id failed");
...
std::string RSA2048::getPublicKeyPEM() const {
    if (!pkey_) throw std::runtime_error("RSA2048::getPublicKeyPEM: no key loaded");

    BIO* bio = BIO_new(BIO_s_mem());
    if (!bio) throw std::runtime_error("RSA2048::getPublicKeyPEM: BIO_new failed");
...
void RSA2048::loadPublicKeyPEM(const std::string& pem) {
    ...
    if (!bio) throw std::runtime_error("RSA2048::loadPublicKeyPEM: BIO_new_mem_buf failed");
...
// ── Encrypt (client, with server public key) ──────────────────────────────────

std::vector<uint8_t> RSA2048::encrypt(const std::vector<uint8_t>& plaintext) const {
    if (!pkey_) throw std::runtime_error("RSA2048::encrypt: no public key loaded");

    EVP_PKEY_CTX* ctx = EVP_PKEY_CTX_new(pkey_, nullptr);
    if (!ctx) throw std::runtime_error("RSA2048::encrypt: EVP_PKEY_CTX_new failed");
...
// ── Decrypt (server, with private key) ───────────────────────────────────────
...
std::vector<uint8_t> RSA2048::decrypt(const std::vector<uint8_t>& ciphertext) const {
    ...
    EVP_PKEY_CTX* ctx = EVP_PKEY_CTX_new(pkey_, nullptr);
    if (!ctx) throw std::runtime_error("RSA2048::decrypt: EVP_PKEY_CTX_new failed");
