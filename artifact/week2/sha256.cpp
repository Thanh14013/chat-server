...
std::string SHA256Hash::hashFile(const std::string& filepath) {
    ...
    if (!file) throw std::runtime_error("SHA256Hash::hashFile: cannot open " + filepath);

    EVP_MD_CTX* ctx = EVP_MD_CTX_new();
    if (!ctx) throw std::runtime_error("SHA256Hash::hashFile: EVP_MD_CTX_new failed");
...
std::vector<uint8_t> SHA256Hash::fromHex(const std::string& hex) {
    if (hex.size() % 2 != 0) throw std::runtime_error("SHA256Hash::fromHex: odd hex length");
