...
bool AdminCommands::hasAdminRight(int adminFd, bool ownerOnly) {
    ...
    ClientSession* s = m_server->getSession(adminFd);
    if (!s) return false;
    if (ownerOnly) return s->role() == UserRole::OWNER;
    return s->role() == UserRole::ADMIN || s->role() == UserRole::OWNER;
}

void AdminCommands::kick(int adminFd, const std::string& targetNick,
    ...
    auto allRooms = m_server->getRoomList();
    for (auto& room : allRooms) {
        for (auto* s : m_server->getSessionsInRoom(room)) {
            if (s->nickname() == targetNick) { target = s; break; }
        }
        if (target) break;
    }
...
void AdminCommands::mute(int adminFd, const std::string& targetNick,
                          int durationSec) {
    if (!hasAdminRight(adminFd)) {
        ClientSession* admin = m_server->getSession(adminFd);
        if (admin) admin->sendPacket(
    ...
    std::string msg = targetNick + " has been muted for "
                    + std::to_string(durationSec) + " seconds by " + anik;
...
void AdminCommands::unmute(int adminFd, const std::string& targetNick) {
    if (!hasAdminRight(adminFd)) return;

    for (auto& room : m_server->getRoomList()) {
        for (auto* s : m_server->getSessionsInRoom(room)) {
            if (s->nickname() == targetNick) {
                s->setMuted(false, 0);
...
void AdminCommands::ban(int adminFd, const std::string& targetNick,
                         const std::string& reason) {
    if (!hasAdminRight(adminFd)) {
        ClientSession* admin = m_server->getSession(adminFd);
        if (admin) admin->sendPacket(
    ...
    AUDIT_D(AuditEventType::ADMIN, anik, targetNick, "BAN",
            AuditResult::SUCCESS, ip, reason);

    m_server->broadcastToRoom(target->currentRoom(),
        Builder::makeSystemNotify(targetNick + " has been banned by " + anik));
    ...
    for (auto& room : m_server->getRoomList()) {
        for (auto* s : m_server->getSessionsInRoom(room)) {
            if (s->nickname() == targetNick) {
                ...
                s->sendPacket(Builder::makeSystemNotify("You have been promoted to ADMIN."));
                AUDIT(AuditEventType::ADMIN, adminNick(adminFd), targetNick,
...
void AdminCommands::demote(int adminFd, const std::string& targetNick) {
    if (!hasAdminRight(adminFd, true)) return;

    for (auto& room : m_server->getRoomList()) {
        for (auto* s : m_server->getSessionsInRoom(room)) {
            if (s->nickname() == targetNick) {
                s->setRole(UserRole::USER);
                s->sendPacket(Builder::makeSystemNotify("Your admin role has been revoked."));
                AUDIT(AuditEventType::ADMIN, adminNick(adminFd), targetNick,
